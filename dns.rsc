/ip dns static
add address=162.159.204.68 name=api.cloudflare.com
add comment=County forward-to=198.18.0.1 regexp=".*(\\.)\?(.*|\\.)\?\\.(hk|cu|\
    at|ca|nz|br|jp|in|tw|mo|ph|vn|tr|my|sg|it|uk|us|kr|ru)\$" type=FWD
...
add comment="Public CDN" forward-to=198.18.0.1 regexp=\
    ".*(\\.)\?(akamaized|akamaihd|cloudfront|cloudflare|hinet).*" type=FWD
...
add comment=Company forward-to=198.18.0.1 regexp=".*(\\.)\?(hp|hpe|ibm|dell|em\
    c|vmware|dellemc|oracle|intel|amd|appdomain)\\.(cloud|com)\$" type=FWD
add comment=Company forward-to=198.18.0.1 regexp=\
    ".*(\\.)\?(nvidia|qualcomm|cisco|arubanetworks|adobe)\\.com\$" type=FWD
add comment=Cloud forward-to=198.18.0.1 regexp=".*(\\.)\?(oraclecloud|alicloud\
    |salesforces|sap|workday|cybersource)\\.com\$" type=FWD
...
add comment=MicroSoft forward-to=198.18.0.1 regexp=\
    ".*(\\.)\?(azure|bing|live|outlook|msn|surface)\\.(net|com|org)" type=FWD
...
add comment=Twitch forward-to=198.18.0.1 regexp=\
    ".*(\\.)\?(twitch|ttvnw|plex).*(\\.net|\\.tv)" type=FWD
...
add comment=Speedtest forward-to=198.18.0.1 regexp=\
    ...
    type=FWD
add comment=Speedtest forward-to=198.18.0.1 regexp=\
    ".*(\\.)\?(speedtest|hgc)\\.(net|com|hk)" type=FWD
...
add comment=Get forward-to=198.18.0.1 regexp=\
    ".*(\\.)\?(medium|wikipedia|archive)\\.(com|org)" type=FWD
...
add comment=News forward-to=198.18.0.1 regexp=\
    ".*(\\.)\?(theinitium|rfa|ntdtv|epochtimes|dw|hk01)\\.(com|org)" type=FWD
...
add comment=Tools forward-to=198.18.0.1 regexp=\
    ".*(\\.)\?(shadowsocks|v2ray|putty|wireguard)\\.(org|com)" type=FWD
add comment=Tools forward-to=198.18.0.1 regexp=\
    ".*(\\.)\?(draw|kubernetes|k8s)\\.io\$" type=FWD
add comment=VPN forward-to=198.18.0.1 regexp=\
    ".*(\\.)\?(nfcloud|v2ray)\\.(net|com)" type=FWD
...
add comment=DisneyPlus forward-to=198.18.0.1 regexp=".*(\\.)\?(bamgrid|disney-\
    plus|disneyplus|dssott|registerdisney.go).*(\\.net|\\.com)" type=FWD
